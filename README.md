...
В этом задании нужно было добавить кеширование и rate limiting к уже существующему API из ДЗ №4.
...
| Коллекция      | Описание                              |
|----------------|---------------------------------------|
| `users`        | Профили пользователей для поиска      |
| `wall_posts`   | Записи на стене                       |
| `chat_messages`| Личные сообщения между пользователями |
...
Запустила нагрузочный тест на кеш (20 запросов подряд) и rate limiter (40 запросов подряд), потом смотрю в Grafana.
